	...
	При возникновении проблем обращайтесь в тг мне:
			tg @ni33er
	...
	cmdBind = {
		...
		[24] = {
			cmd = "/hlp",
			key = {},
			desc = "Лечение на порту с автоматической отыгровкой",
			rank = 2,
			rb = false
		},
			[25] = {
	...
	/medcard #playerID #healID #timeID 1000]]  
	  ...
	  while true do
		...
		if thread:status() ~= "dead" and not isGamePaused() then 
			renderFontDrawText(fontPD, "Отыгровка: [{F25D33}Page Down{FFFFFF}] - Приостановить её", 20, sy-30, 0xFFFFFFFF)
		...
		if mainWin.v then
			...
			imgui.Begin(fa.ICON_HEARTBEAT .. " MedicalHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
				...
				imgui.BeginGroup()
					...
					imgui.Text(u8"Информация о сотруднике больницы");
				...
				imgui.BeginChild("settig", imgui.ImVec2(0, 390), true)
					imgui.Text(fa.ICON_ANGLE_RIGHT .. u8" Данный раздел предназначен для полной настройки скрипта под себя");
					imgui.Separator()
					imgui.Dummy(imgui.ImVec2(0, 5))
				...
				if select_menu[7] then
					...
					imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
						...
						imgui.Text(u8"Medical Helper 2")
						imgui.Spacing()
						imgui.TextWrapped(u8"\tСкрипт был разработан для проекта Ariona Role Play с поддержкой работы на сервере Glendale для облегчения работы сотрудникам больниц. Благодаря этому приложению Вы получите полный комплекс автоматизации многих действий и наслаждение от пользования.\nОбновления выходят по мере добавления нововведений и исправлений ошибок.")
						imgui.Dummy(imgui.ImVec2(0, 10))
						imgui.Bullet()
						imgui.TextColoredRGB("Разработчик - {FFB700}Anarch")
						imgui.Bullet()
						imgui.TextColoredRGB("Версия скрипта - {FFB700}".. scr.version)
						imgui.Bullet()
						imgui.TextColoredRGB("Огромная благодарность blast.hk за методическую информацию и его пользователям в решении вопросов.")
							...
							imgui.TextColoredRGB("Желающие поддержать разработку денюшкой - ".."{68E15D}\"Открыть\"")
								...
								if imgui.IsItemClicked(0) then print(shell32.ShellExecuteA(nil, 'open', 'https://www.donationalerts.com/r/anarch', nil, nil, 1)) end
		...
		if thread:status() == "dead" then
			...
			for k, v in pairs(cmdBind) do
				if sKeys == tostring(table.concat(v.key, " ")) then
					...
					elseif k == 11 then
						funCMD.osm()
					elseif k == 12 then -- деп
						depWin.v = not depWin.v
					elseif k == 13 then -- соб
					...
					elseif k == 24 then
						...
						elseif k == 25 then
						 if resTarg then
							funCMD.strax(tostring(targID))
						else
							sampSetChatInputEnabled(true)
							sampSetChatInputText("/givemedinsurance ")
						end
						elseif k == 26 then
						 if resTarg then
							funCMD.antib(tostring(targID))
						else
							sampSetChatInputEnabled(true)
	...
	function funCMD.lec(id)
		...
		if id:find("%d+") then
			thread = lua_thread.create(function()
				if not isCharInModel(PLAYER_PED, 416) then
					sampSendChat(string.format("Здравствуйте. Я сотрудник данного медицинского центра, что вас беспокоит?", u8:decode(buf_nick.v)))
					wait(1000)
		...
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Используйте команду /hl [id игрока] [Цена].", 0xEE4848)
		end
	end
	function funCMD.hlp(id)
		if thread:status() ~= "dead" then
			sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: В данный момент проигрывается отыгровка.", 0xEE4848)
			return
		end
		...
		if id:find("%d+") then
			thread = lua_thread.create(function()
				if not isCharInModel(PLAYER_PED, 416) then
					sampSendChat(string.format("/do В руке у %s подсумок BLS, со всеми нужными медикаментами.", u8:decode(buf_nick.v)))
					wait(2000)
					sampSendChat(chsex("/me присел на колено, открыл подсумок, вынул из него нужный препарат", "/me присела на колено, открыла подсумок, вынула из него нужный препарат"))
					wait(2000)
					sampSendChat(chsex("/me бегло осмотрел человека, ощупал возможные места кровотечений.", "/me бегло осмотрела человека, ощупала возможные места кровотечений."))
					wait(2000)
					sampSendChat("/todo Бери давай, бегом-бегом.*передав препарат человеку напротив")
					wait(2000)
					sampSendChat(chsex("/me закрыл подсумок, закинул его на плечо", "/me закрыла подсумок, закинула его на плечо"))
					wait(100)
					sampSendChat("/heal "..id)
				end
			end)
	...
	if thread:status() ~= "dead" then
		...
		if id:find("%d+") then
			thread = lua_thread.create(function()
				if not isCharInModel(PLAYER_PED, 416) then
					sampSendChat(string.format("Приветствую.", u8:decode(buf_nick.v)))
					wait(2000)
					sampSendChat("Вы хотите получит антибиотики?")
					sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на  {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
						addOneOffSound(0, 0, 0, 1058)
						local len = renderGetFontDrawTextLength(font, "{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить")
						while true do
							wait(0)
							renderFontDrawText(font, "Антибиотики: {8ABCFA}Ответ\n{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить", sx-len-10, sy-50, 0xFFFFFFFF)
							if isKeyJustPressed(VK_RETURN) and not sampIsChatInputActive() and not sampIsDialogActive() then break end
						end
					sampSendChat("Хорошо,в каком количестве?")
					wait(500)
					sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на  {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
						addOneOffSound(0, 0, 0, 1058)
						local len = renderGetFontDrawTextLength(font, "{FFFFFF}[{67E56F}Enter{FFFFFF}] - Продолжить")
					...
					sampSendChat("Отлично,подождите немного,сейчас я все подготовлю.")
					wait(2000)
					sampSendChat(chsex("/me осторожным движением правой руки открыл шкафчик ресепшена", "/me осторожным движением правой руки открыла шкафчик ресепшена"))
					wait(2000)
					sampSendChat("/do Шкафчик открыт.")
					wait(2000)
					sampSendChat("/me Правой рукой взял лист бумаги на котором написано 'Антибиотики' ")
					wait(2000)
					sampSendChat("/do Лист бумаги на котором написано 'Антибиотики' в правой руке.")
					wait(2000)
					sampSendChat("Сейчас,я дам вам ручку и бумагу вы заполните количество антибиотиков и поставите роспись.")
					wait(2000)
					sampSendChat("/todo Вот держите*передовая ручку и бумагу обращаясь к пациенту.")
					wait(2000)
					sampSendChat("/n Используя отыгровки через /me и /do.")
					wait(2000)
					sampSendChat(chsex("/me взял лист бумаги и ручку у пациента в левую руку", "/me взяла лист бумаги и ручку у пациента в левую руку"))
					wait(2000)
					sampSendChat("/do Лист бумаги и ручку в левой руке.")
					wait(2000)
					sampSendChat(chsex("/me положил лист бумаги и ручку в шкафчик,после чего закрыл его", "/me положила лист бумаги и ручку в шкафчик,после чего закрыла его"))
					wait(2000)
					sampSendChat("Сейчас я вам выдам антибиотики.")
					wait(2000)
					sampSendChat(chsex("/me открыл правой рукой мед.кейс", "/me открыла правой рукой мед.кейс"))
					wait(2000)
					sampSendChat("/do Мед.кейс открыт.")
					wait(2000)
					sampSendChat(chsex("/me левой рукой достал нужное количество антибиотиков и передав их пациенту", "/me левой рукой достала нужное количество антибиотиков и передав их пациенту"))
					wait(2000)
	...
	function funCMD.strax(id)
		...
		if id:find("%d+") then
			thread = lua_thread.create(function()
				if not isCharInModel(PLAYER_PED, 416) then
					sampSendChat("Хорошо, тогда давайте обсудим некоторые формальностей и продолжим.")
					wait(2000)
					sampSendChat("Значит, Вам потребуется поставить подпись под согласие на обработку ваших персональных данных.")
					wait(2000)
					sampSendChat("/todo Вы согласны? Вот документ, можете ознакомиться* указывая на бумажку на столе")
					wait(2000)
					sampSendChat("/todo Хорошо, раз вы ознакомлены, и согласны распишитесь тут и продолжим* указывая пальцем на место под роспись")
					wait(500)
					sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
						while true do
						wait(0)
					...
					sampSendChat("/me передал паспорт пациенту")
					wait(2000)
					sampSendChat("/me ухватился за мед.карту с рук человека, после открыл ее и начал рассматривать страницы")
					wait(2000)
					sampSendChat("У нас имеется три вида страхования,первый вид выдается на одну неделю,цена 400.000$,вторая выдается....")
					wait(2000)
					sampSendChat("....на две недели,цена 800.000$,третья выдается на три недели,цена 1.200.000$.")
					wait(2000)
					sampSendChat("Какую вы хотите приобрести?")
					wait(500)
					sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Нажмите на {23E64A}Enter{FFFFFF} для продолжения.", 0xEE4848)
						while true do
						wait(0)
					...
					sampSendChat("/todo Отлично. Раз вы все-таки готовы, можем приступать* беря с ящика пустые книжки мед.страхования")
					wait(2000)
					sampSendChat("Ладно, сэр, попрошу, держите ваш паспорт с мед.картой так, чтобы я видел их данные, пожалуйста.")
					wait(2000)
					sampSendChat("/todo О-о-отлично, почти заполнил.* записывая информацию о человека с документов в книжку")
					wait(2000)
					sampSendChat("/todo Хорошо, все готово, сейчас поставлю печать и можете оплатить пошлину* берясь за печатный станок")
					wait(2000)
					sampSendChat("/me поставил печать: 123.")
					wait(2000)
					sampSendChat("/todo Отлично, сэр. Теперь можете идти оплачивать гос.пошлину и все будет готово.* отдавая в руки книжку и личном мед.страховании")
					wait(2000)
					sampSendChat("/do Страховка в левой руке.")
					wait(2000)
					sampSendChat("/me передал страховой полис человеку на против")
					wait(1000)
					sampSendChat("/givemedinsurance")
				end
			end)
		else
		sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Используйте команду /strax [id игрока]	.", 0xEE4848)
		end
	end
