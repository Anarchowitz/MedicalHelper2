...
script_authors("Hatiko (Remade by Hiroto)")
			...
			if select_menu[7] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.TextColoredRGB("Благодарность blast.hk за методическую информацию и его пользователям в решении вопросов.")
						...
						imgui.SameLine()
						imgui.TextColoredRGB("Для связи: TG: {74BAF4}@ni33er") --или Discord - {74BAF4}TheVitek#2160")
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				...
				addOneOffSound(0, 0, 0, 1058)
				sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Произошла ошибка при скачивании обновления [WARN - Так и задумано!]", 0xEE4848)
...
function funCMD.updateCheck()
	sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Проверяем наличие обновлений...", 0xEE4848)
		...
		local url = "https://raw.githubusercontent.com/Anarchowitz/MedicalHelper2/master/update.txt"
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
					wait(1000)
